...
- **The tool argument is the BARE action slug** — `YOUTUBE_VIDEOS_LIST`, not
  `composio.YOUTUBE_VIDEOS_LIST`. The `composio.` namespace is only how Core's
  `McpRegistry` addresses a Composio action inside its own tool namespace; the
  namespace is not part of the action's bare name and Composio's execute URL would
  404 on it.
