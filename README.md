...
UGC campaign tracker for Ryu — run creator-marketing campaigns end to end: briefs and budgets, a creator roster, post submissions with approve/reject review, post metrics refreshed through a curated Composio action map, and CPM/flat-rate payouts accrued, approved and marked paid.
...
- Binary: `ryu-ugc` from the [Ryu releases](https://github.com/amajorai/ryu/releases).
- Crate: `cargo install ryu-ugc`.
...
Creator-marketing campaign tracking. A **campaign** carries a brief, the platforms it
targets, required hashtags/mentions, a window, a budget, and a payout rule (CPM per 1k
views, or flat per approved post, plus optional bonus tiers). **Creators** are the
roster. A **submission** binds one creator's post to one campaign; it is reviewed, and
on approval it starts accruing. **Metric snapshots** are the cross-run state that makes
a submission more than a link: each refresh appends a reading of views/likes/comments,
and the accrual pass re-prices that submission's single payout row from the **latest**
snapshot. Everything settles in integer cents against the campaign budget.

Metrics come back through a **curated platform → Composio action map** — five rows
(YouTube, TikTok, Instagram, X, LinkedIn), each naming one action id, the argument that
carries the post id, and the dotted selectors for each counter. A campaign never names
an action.
