...
//!
//! That is a deliberate correction. The app used to hand the slug to Core's
//! `mcp.callTool` kernel capability as the MCP tool id `composio__<SLUG>` (the
//! prefix `McpRegistry` strips before dispatching to its Composio provider). The
//! capability's ROUTING is generic but its HANDLER is not: `mcp.callTool` lands in
//! `monitors_client::host_spider_crawl`, which returns 403 "not the monitors app"
//! for every caller that is not `@ryu/monitors`
//! (`apps/core/src/monitors_client.rs:314`). The declared `tools.invoke` grant
//! passes and the handler then refuses, so that hop could never succeed for
//! `@ryu/ugc` — no prefix, no arguments, nothing on this side changes a check keyed
//! on our own plugin id. `composio__` therefore no longer appears anywhere here.
//!
...
mod tests {
    ...
    /// own execute endpoint is addressed with, NOT the `composio__`-prefixed MCP
    /// id, which only ever meant something to Core's registry and is gone.
    #[test]
    fn curated_actions_are_bare_slugs_with_no_mcp_prefix() {
        for src in PLATFORM_METRIC_SOURCES {
            ...
            assert!(
                !action.contains("composio__"),
                "'{action}' still carries the retired MCP prefix"
            );
        }
    }
    ...
    fn key_precondition_names_this_apps_own_settings_not_cores() {
        ...
        assert!(err.to_string().contains("/api/ugc/settings/composio-key"), "{err}");
    ...
    fn sample_from_refuses_a_response_that_carries_none_of_its_counters() {
        for body in [
            ...
            assert!(err.to_string().contains("without any of the counters"), "{err}");
        }
    }
    ...
    async fn fetch_metrics_refuses_a_platform_with_no_curated_source() {
        let err = fetch_metrics(&Client::new(), "myspace", "1")
            ...
            .unwrap_err();
        assert!(matches!(err, MetricError::Unsupported(_)));
        assert!(err.to_string().contains("no Composio metric source"), "{err}");
    }
