...
//! # Provider execution
//!
//! Metric refreshes use the provider-neutral `ProviderRouter` bridge. Core
//! authenticates this sidecar and Gateway supplies the managed Composio credential
//! and records the provider cost against the organization wallet. No provider key
//! enters this crate or its data directory.
//!
...
/// Marker for the process hosting the UGC engine. Provider credentials are not
/// part of this contract: metric calls use `ProviderRouter` and Ryu's Gateway.
...
pub enum RefreshError {
    ...
    /// Ryu's managed Composio provider is unavailable or the curated source is
    /// missing — 400, with the message naming the fix and telling the user to
    /// record metrics by hand meanwhile.
    Unsupported(String),
...
pub struct UgcEngine {
    ...
    /// The client used by the managed provider bridge and app-event emitter.
    http: reqwest::Client,
    /// Provider-neutral bridge to Ryu Core/Gateway. Provider credentials and
    /// wallet debits stay outside this satellite.
    provider_router: ryu_app_events::ProviderRouter,
...
impl UgcEngine {
    ...
    pub fn new(store: UgcStore, http: reqwest::Client, host: Arc<dyn UgcHost>) -> Self {
        Self {
            ...
            provider_router: ryu_app_events::ProviderRouter::with_client(
                UGC_PLUGIN_ID,
                http.clone(),
            ),
            http,
    ...
    /// Whether Ryu's managed Gateway has a Composio provider available. The
    /// credential itself never crosses into this process.
    pub async fn managed_provider_configured(&self, provider: &str) -> bool {
        self.provider_router.status(provider).await.unwrap_or(false)
    }
        ...
        if outcome == WriteOutcome::Written {
            self.events
                .emit_with_notify(
                    ...
                    // A new creator submission is review work waiting on the user —
                    // exactly what the Inbox is for.
                    Some(ryu_app_events::NotifyHint::info(
                        "New submission to review",
                        Some(format!("{} submitted on {}", s.creator_id, s.platform)),
                    )),
                )
                .await;
        }
...
pub(crate) mod testutil {
    ...
    /// A fully in-memory host marker. Provider credentials are supplied by the
    /// managed Gateway bridge, so tests never need a fake key lifecycle.
    #[derive(Default)]
    pub struct FakeHost;
    ...
    /// An engine over a fresh temp store with a default host marker.
...
mod tests {
    ...
    async fn accrual_reprices_in_place_and_never_doubles_spend() {
        ...
        assert_eq!(
            payouts.len(),
            1,
            "one row per submission, re-priced in place"
        );
    ...
    async fn rejecting_removes_the_accrued_payout() {
        ...
        assert!(out.changed);
        assert_eq!(
            out.submission.rejection_reason.as_deref(),
            Some("missing the hashtag")
        );
        assert!(
            engine
                .store
    ...
    /// fetch talks to the managed Composio provider (so no fake host can stand in for it): a
    ...
    async fn a_needs_connection_outcome_writes_no_snapshot_and_reprices_nothing() {
        ...
        assert!(
            matches!(outcome, RefreshOutcome::NeedsConnection { ref message, .. }
    ...
    async fn campaign_refresh_reports_per_submission_and_never_fails_the_batch() {
        ...
        assert!(report
            .results
            .iter()
            .all(|r| r.status == RefreshStatus::Error
                && r.snapshot.is_none()
                && r.message.is_some()));
