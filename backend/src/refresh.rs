...
/// from Composio directly now, so there is no host callback left to count calls
...
async fn run_once(engine: &UgcEngine) -> Vec<String> {
    let campaigns = match engine.store.list_campaigns(Some(CampaignStatus::Active)).await {
        Ok(c) => c,
        Err(e) => {
...
mod tests {
    ...
    use ryu_ugc::{
        Campaign, ComposioKeySource, PayoutRule, Submission, SubmissionStatus, UgcHost, UgcStore,
        DB_FILE_NAME,
    };

    /// The smallest thing that satisfies [`UgcHost`], which is now only "where is
    /// the app's Composio key persisted". The loop never touches the key, and the
    /// lib's own `testutil::FakeHost` is `#[cfg(test)] pub(crate)` so the bin crate
    /// cannot see it.
    ///
    /// It deliberately does NOT call [`ryu_composio::auth::set_key`]: that cache is
    /// process-global and shared with `main.rs`'s key-lifecycle tests in this same
    /// binary, so a test that set a key would change what every other test reads.
    struct NoKeyHost;

    impl UgcHost for NoKeyHost {
        fn set_composio_key(&self, _key: &str) -> Result<ComposioKeySource, String> {
            Err("the refresh loop never sets a key".to_string())
        }
        fn clear_composio_key(&self) -> Result<ComposioKeySource, String> {
            Ok(ComposioKeySource::None)
        }
    ...
    /// metrics are fetched from Composio directly — a refreshable row would try to
    /// reach the real API.
    ...
    async fn a_tick_walks_active_campaigns_only() {
        ...
        for (id, status) in [
            ...
            engine.store.upsert_campaign(&campaign(id, status)).await.unwrap();
        }
